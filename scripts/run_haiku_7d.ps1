<#
  Corre el experimento unos pocos dias con un modelo barato/rapido (por defecto
  Claude Haiku) para ver como se comporta antes de gastar en una corrida completa.
  ...
  Uso (desde la raiz del repo o desde cualquier lado):
    .\scripts\run_haiku_7d.ps1
    .\scripts\run_haiku_7d.ps1 -Dias 3
    .\scripts\run_haiku_7d.ps1 -Modelo claude-haiku-4-5 -Dias 7
#>
param(
...
if ($LASTEXITCODE -eq 0) {
  ...
  Copy-Item log.json "samples/log_haiku_${Dias}d.json" -Force
  Write-Host "[The Last Million] Listo. Log activo en log.json (+ visualizer/public/log.json)." -ForegroundColor Green
  Write-Host "[The Last Million] Copia guardada en samples/log_haiku_${Dias}d.json" -ForegroundColor Green
